...
st.write("已完美融會貫通 84 頁商業配方與 176 頁醫院常用治療伙食指南，低/中/高脂肉類全規格高調歸位！")
...
DIET_DB = {
    ...
    "限制熱量減重飲食 (Low Calorie)": {"kcal_kg": 20, "pro_g": 1.2, "cho_ratio": 50, "notes": "每日設定 500-1000 kcal 熱量赤字。限制高飽和脂肪肉品，SAFA≤10%。", "residue": "常規一般飲食"},
    ...
    "慢性肝病/肝硬化照護飲食": {"kcal_kg": 32, "pro_g": 1.1, "cho_ratio": 62, "notes": "🚨必須少量多餐，且強迫安排『夜間點心 (Late-night snack)』選用低氮澱粉或黃豆 BCAA 點心阻斷夜間骨骼肌自噬分解！", "residue": "常規一般飲食"},
...
st.sidebar.markdown("---")
st.sidebar.header("🧬 第五步：天然肉類與餐次微調")

# 🚀 重新拉回三大肉類規格（低、中、高脂全面歸位連動）
meat_type = st.sidebar.selectbox("天然優質肉類分級選擇:", ["低脂肉類 (P:7g, F:3g)", "中脂肉類 (P:7g, F:5g)", "高脂肉類 (P:7g, F:10g)", "不吃肉類/無肉品"])

if "低脂" in meat_type:
    meat_p, meat_f = 7.0, 3.0
    meat_ref = "生重 35 克低脂肉類 (如一般魚類 35 克、無糖豆漿 190 毫升)"
elif "中脂" in meat_type:
    meat_p, meat_f = 7.0, 5.0
    meat_ref = "生重 35 克中脂肉類 (如豆腐 80 克、雞蛋 55 克)"
elif "高脂" in meat_type:
    meat_p, meat_f = 7.0, 10.0
    meat_ref = "生重 35 克高脂肉類 (如百頁豆腐 70 克、富含飽和脂肪酸五花肉)"
else:
    meat_p, meat_f = 0.0, 0.0
...
if residue_type == "無渣飲食 (完全清空/嚴禁乳品)":
    ...
    milk_p, milk_f, milk_c, milk_servings = 0.0, 0.0, 0.0, 0.0
    veg_servings, fruit_servings = 1.5, 1.0
else:
    milk_p, milk_f, milk_c, milk_servings = 8.0, 4.0, 12.0, 1.0 # 預設低脂乳1份
...
# ==================== UI Output Dashboard ====================
st.subheader(f"🩺 臨床醫院治療伙食：{selected_diet} (當前選用：{selected_formula} $\\times$ {cans} 罐)")
...
# 額外警報卡片
if "高血脂" in selected_diet and "高脂" in meat_type:
    st.error("🚨 **高危臨床介入錯誤警告**：高血脂症個案在追求高纖的同時，**嚴禁搭配高脂肉類**！否則高飽和脂肪酸將下調肝臟 LDL 受體轉錄，導致血脂大崩盤！")
elif "CKD" in selected_diet and "高脂" in meat_type:
    st.warning("⚠️ **腎病高磷風險警告**：高脂肉常伴隨高加工無機磷（Inorganic Phosphorus），在遠端腎小管排泄受阻下會誘發高血磷，請謹慎使用。")

st.markdown("### 🍽️ 淨天然食物：六大類食物每日精準總份數 (已全面扣除商業配方與肉類油脂分攤)")
food_groups_data = {
    ...
    "臨床生重與品項精確指引": [
        "精製穀類首選。若為低渣/無渣/CKD/糖腎，全面改用白米飯、冬粉、西谷米或粉飴！",
        f"當前選用：{meat_ref}。一經切換，下方『油脂堅果類』會自動扣除對應油脂克數，嚴防超標！",
        "無渣及低渣飲食已自動關閉乳品類。常規成人選用低脂乳1份 (240ml)。",
        "一份生重 100g。限鉀個案切記：必須先切、後充分汆燙 3-5 分鐘，瀝乾去汁後方可烹調！",
        "一份約 100g。糖腎、CKD、HD、CAPD 必須控管在 2 份內，避開高鉀高糖地雷水果。",
        f"約 {fat_servings*5:.1f} 克烹調油。若上方選用『高脂肉』，此處份數會因為油脂預算被分攤而自動調降！"
    ]
}
...
# 全自動化產出處方
st.markdown("---")
st.subheader("📋 結合 ONS 與天然肉類分級扣除法之全自動臨床營養處方 (Nutrition Prescription)")
pres_text = f"【臨床處方】每日給予該個案『{selected_diet}』之天然常規餐點（搭配 {meat_type} 質地）共 {tdee:.0f} 大卡（內含蛋白質 {target_pro_g:.1f}g）；並經由管灌管道每日固定分次注入 {selected_formula} 共 {cans} 罐（占熱量 {en_kcal:.0f} kcal），餐與餐之間需外加強迫追加沖洗純水共 {gap_water:.0f} 毫升，以維體液與電解質平衡。"
